...
const MemoryGauge = ({ usedGb, wiredGb, totalGb }: MemoryGaugeProps) => {
  ...
  const pressure = usedPct > 85 ? "Critical" : usedPct > 70 ? "Warning" : "Normal";
  const pressureColor = usedPct > 85 ? "text-crimson" : usedPct > 70 ? "text-amber" : "text-emerald";
      ...
      <div className="flex items-center justify-center">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-2xl font-mono font-semibold text-foreground">{usedGb.toFixed(1)}</span>
      ...
      <div className="mt-4 space-y-2">
        <div className="flex items-center justify-between text-xs">
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full bg-electric-blue" /> App Memory
          </span>
          <span className="font-mono text-foreground">{(usedGb - wiredGb).toFixed(1)} GB</span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="flex items-center gap-1.5">
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <span className={`font-mono font-medium ${pressureColor}`}>{pressure}</span>
        </div>
      </div>
    </div>
  );
