  ...
  return (
    <div className="glass-card-hover p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`status-badge text-[10px] inline-flex items-center gap-1.5 ${status.ai_status === "inferring" ? "bg-emerald/10 text-emerald" : "bg-secondary text-muted-foreground"}`}>
          <span className={`w-1.5 h-1.5 rounded-full bg-current ${status.ai_status === "inferring" ? "pulse-dot" : ""}`} />
      ...
      <div className="bg-background rounded-lg border border-border overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="flex items-center">
          ...
          <input
            ...
            disabled={generating}
          />
          <button type="submit" disabled={generating || !prompt.trim()} className="p-3 min-h-10 min-w-10 flex items-center justify-center text-muted-foreground hover:text-primary transition-colors disabled:opacity-30">
